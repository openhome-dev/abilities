...
class TwilioSmsCapability(MatchingCapability):
    ...
    async def load_prefs(self):
        ...
        try:
            ...
            if exists:
                # 1 & 2. Use await and set temp=False for persistent storage
        ...
        # If the file does not exist or is empty (first run), create a clean template
        if not self.prefs:
            self.prefs = {
                "account_sid": "",
                "auth_token": "",
    ...
    async def run(self):
        ...
        try:
            # Wait for prefs to load asynchronously
