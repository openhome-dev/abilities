...
EXIT_WORDS = ["done", "that's it", "thats it", "exit", "stop", "quit",
              "go to sleep", "goodbye", "bye", "nothing else", "all good",
              "nope", "no thanks", "i'm good", "im good"]
...
class OutlookCapability(MatchingCapability):
    ...
    pending_create: dict = None  # Tracks create flow: {"title": "...", "waiting_for": "title|time"}
    pending_invite: dict = None  # Tracks invite flow: {"event": ..., "waiting_for": "email|event"}
    calendar_timezone: str = "America/New_York"  # Default, will be overridden by calendar data
    ...
    def execute_tool(self, tool_slug, params):
        ...
        try:
            ...
            if tool_slug == "OUTLOOKCALENDAR_FIND_EVENT":
                ...
                for event in items:
                    ...
                    normalized_items.append({
                        ...
                        "start": {
                            "dateTime": trim_iso(event.get("start", {}).get("dateTime")),
                            "timeZone": start_tz or "UTC"
                        },
                        "end": {
                            "dateTime": trim_iso(event.get("end", {}).get("dateTime")),
            ...
            if tool_slug == "OUTLOOKCALENDAR_CREATE_EVENT":
                ...
                duration_minutes = (
                    params.get("event_duration_hour", 0) * 60 +
                    params.get("event_duration_minutes", 0)
                )
                ...
                if params.get("location"):
                    body["location"] = {
                        "displayName": params.get("location")
                    }
    ...
    def get_trigger_context(self):
            ...
            if not history:
                ...
                for i, msg in enumerate(history[-5:]):
                    try:
                        ...
                        role = str(msg.role).lower() if hasattr(msg, 'role') else 'unknown'
                        self.log(f"History[{i}]: type={msg_type}, role={role}, content={content_preview}")
                    except Exception as e:
                ...
                for msg in reversed(history):
                    try:
                        ...
                        if not content or '[HUB_SESSION_END|' in content or '[SYSTEM CONTEXT]' in content:
                            continue
        ...
        self.log(f"Trigger context: {len(recent_user_messages)} messages, trigger: '{trigger_message[:80] if trigger_message else 'none'}'")
    ...
    def classify_trigger_intent(self, trigger_context: dict):
        ...
        full_triggers = ["catch me up", "smart hub", "what's going on", "brief me", "run through my day", "overview"]
        if any(ft in trigger.lower() for ft in full_triggers):
            self.log("Full briefing trigger detected")
            return {
                "intent": "full_briefing",
                "mode": "full",
                "details": {}
            }
        ...
        try:
            ...
            self.log(f"Trigger classification: intent={result.get('intent')}, mode={result.get('mode')}, details={result.get('details')}")
            return result
        except Exception as e:
            ...
            lower = trigger.lower()
            if any(w in lower for w in ["what's on", "schedule", "calendar today", "my day"]):
                return {"intent": "read_today", "mode": "quick", "details": {}}
            elif any(w in lower for w in ["create", "schedule a", "set up", "new meeting", "new event"]):
    ...
    def fetch_weather(self, lat, lon, use_fahrenheit=True):
        ...
        try:
            ...
            return {
                ...
                "condition": WEATHER_DESCRIPTIONS.get(weather_code, "unclear conditions"),
    ...
    def collect_geo_context(self):
        ...
        self.log(f"Geo context: {city}, {region} | {time_bucket} | {weather.get('temp')}°")
    ...
    def has_in_person_meeting(self):
        ...
        for event in self.context.get("calendar", []):
            ...
            if location and not any(x in location.lower() for x in ["http", "zoom", "meet.google", "teams.microsoft"]):
                return True
        return False
    ...
    def fetch_upcoming_today(self):
        ...
        if user_tz:
            ...
            tz_offsets = {
                ...
                "Pacific/Honolulu": -10, "America/Detroit": -5,
            ...
            hours_ahead = min(hours_until_midnight + 1, 14)  # Cap at 14 hours, add 1 for buffer
        ...
        if items:
            ...
            has_local_offset = start_dt and ('+' in start_dt[-6:] or start_dt[-6:-5] == '-')
    ...
    def get_content_words(self, text: str) -> set:
        ...
        stop_words = {
            'the', 'a', 'an', 'with', 'my', 'to', 'on', 'at', 'for', 'of', 'in',
            'and', 'or', 'is', 'it', 'can', 'you', 'i', 'me', 'we', 'our',
            'event', 'meeting', 'call', 'sync', 'one', 'that', 'this'
        }
    ...
    def find_event_by_match(self, match_text: str):
        ...
        for event in calendar:
            ...
            for match_word in match_content_words:
                ...
                for title_word in title_content_words:
                    ...
                    if match_word.startswith(title_word[:3]) or title_word.startswith(match_word[:3]):
                    ...
                    # Check if one contains the other
                        ...
                        self.log(f"  Substring match in words: '{match_word}' in '{title_word}' or vice versa")
            ...
            if total_score > best_score:
                ...
                best_match = event
                self.log(f"  '{title}' - score: {total_score} (exact: {content_overlap}, fuzzy: {fuzzy_match_score})")
                ...
                for word in match_content_words:
                    ...
                    if len(word) >= 3:
                        ...
                        for att_word in attendee_words:
                            if len(att_word) >= 3 and (word.startswith(att_word[:3]) or att_word.startswith(word[:3])):
                                self.log(f"  Fuzzy attendee match: '{word}' ~ '{att_word}'")
                                return event
        ...
        if best_score >= 0.5:
            self.log(f"Matched event: '{best_match['title']}' (ID: {best_match.get('id', 'N/A')}) with score {best_score}")
            return best_match
    ...
    def detect_conflicts(self, event_id: str, new_start: str, new_end: str):
        ...
        for event in self.context.get("calendar", []):
            ...
            try:
                evt_start = datetime.datetime.fromisoformat(event["start"].replace("Z", "+00:00"))
                evt_end = datetime.datetime.fromisoformat(event["end"].replace("Z", "+00:00"))
    ...
    def reschedule_event(self, event: dict, minutes_delta: int):
        ...
        try:
            old_start = datetime.datetime.fromisoformat(event["start"].replace("Z", "+00:00"))
            old_end = datetime.datetime.fromisoformat(event["end"].replace("Z", "+00:00"))
            ...
            self.log(f"Reschedule: {event['title']} | duration: {original_duration}m | delta: {minutes_delta}m")
            self.log(f"  Old: {old_start.isoformat()} to {old_end.isoformat()}")
            self.log(f"  New start: {new_start.isoformat()}, duration: {duration_hours}h {duration_minutes}m")
            
            new_end = new_start + datetime.timedelta(minutes=original_duration)
            conflicts = self.detect_conflicts(event["id"], new_start.isoformat(), new_end.isoformat())
            ...
            update_result = self.execute_tool("OUTLOOKCALENDAR_UPDATE_EVENT", update_params)
    ...
    def shorten_event(self, event: dict, new_duration_minutes: int):
        ...
        try:
            ...
            self.log(f"Shorten: {event['title']} to {new_duration_minutes}m ({duration_hours}h {duration_minutes}m)")
            ...
            if update_result:
                self.log(f"Changed '{event['title']}' duration to {new_duration_minutes} minutes")
                return update_result, None
            else:
                return None, "Failed to update event"
    ...
    def create_event(self, title: str, start_time: str, duration_minutes: int, attendees: list = None):
        ...
        try:
            ...
            self.log(f"Creating event '{title}' at {start_dt.isoformat()} for {duration_minutes}m")
    ...
    def cascade_reschedule(self, starting_event: dict, minutes_delta: int):
        ...
        try:
            start_time = datetime.datetime.fromisoformat(starting_event["start"].replace("Z", "+00:00"))
            start_end = datetime.datetime.fromisoformat(starting_event["end"].replace("Z", "+00:00"))
        ...
        current_new_end = start_time + datetime.timedelta(minutes=minutes_delta) + datetime.timedelta(minutes=start_duration)
        ...
        for evt_data in all_events:
            ...
            if evt_data["start"] < current_new_end:
                ...
                new_evt_start = evt_data["start"] + datetime.timedelta(minutes=minutes_delta)
                current_new_end = new_evt_start + datetime.timedelta(minutes=evt_data["duration"])
            else:
        ...
        for evt_data in events_to_move:
            ...
            result, error = self.reschedule_event_no_conflict_check(event, minutes_delta)
            if error:
    ...
    def reschedule_event_no_conflict_check(self, event: dict, minutes_delta: int):
        ...
        try:
            ...
            self.log(f"Cascade move: {event['title']} | {old_start.strftime('%H:%M')} -> {new_start.strftime('%H:%M')}")
    ...
    def parse_time_to_datetime(self, time_str: str):
        ...
        patterns = [
            (r'^(\d{1,2}):(\d{2})(AM|PM)$', lambda m: (int(m.group(1)) % 12 + (12 if m.group(3) == 'PM' else 0), int(m.group(2)))),
            (r'^(\d{1,2})(AM|PM)$', lambda m: (int(m.group(1)) % 12 + (12 if m.group(2) == 'PM' else 0), 0)),
            (r'^(\d{1,2}):(\d{2})$', lambda m: (int(m.group(1)), int(m.group(2)))),
            (r'^(\d)(\d{2})(AM|PM)$', lambda m: (int(m.group(1)) % 12 + (12 if m.group(3) == 'PM' else 0), int(m.group(2)))),
            (r'^(\d{2})(\d{2})(AM|PM)$', lambda m: (int(m.group(1)) % 12 + (12 if m.group(3) == 'PM' else 0), int(m.group(2)))),
        ]
        ...
        for pattern, extractor in patterns:
            ...
            if match:
                ...
                if 0 <= hour <= 23 and 0 <= minute <= 59:
                    ...
                    self.log(f"Parsed time '{original_time_str}' -> {result.isoformat()} (user tz: {self.calendar_timezone})")
                    return result
    ...
    def classify_calendar_intent(self, user_message: str):
        ...
        if self.context.get("calendar"):
            ...
            for e in self.context["calendar"]:
                ...
                lines.append(f"- {status} {e['title']} | {e['start']} to {e['end']} | attendees: {', '.join(e.get('attendees', []))}")
            cal_context = "\n".join(lines)
    ...
    async def handle_calendar_write(self, intent_data: dict, user_message: str):
        ...
        self.log(f"Calendar write intent: {intent} | match: {event_match} | mins: {change_mins}")
        ...
        is_ambiguous = (
            not event_match or 
            "ambiguous" in event_match.lower() or
            "asking which" in event_match.lower() or
            "unclear" in event_match.lower() or
            "user asking" in event_match.lower() or
            len(event_match) > 60  # LLM explanation, not a real match
        )
        ...
        if event:
            self.log(f"Matched event: '{event['title']}' (ID: {event.get('id', 'none')})")
        else:
        ...
        if intent in ["reschedule", "shorten", "extend", "cancel"] and not event:
            ...
            if calendar and len(calendar) <= 3:
                event_names = [self.clean_title_for_speech(e.get("title", "Untitled")) for e in calendar]
        ...
        if intent == "reschedule":
            ...
            if change_mins < 0:
                ...
                return f"Done! I've pushed '{event['title']}' back {change_mins} minutes."
            ...
            try:
                ...
                conflicts = self.detect_conflicts(event["id"], event["start"], new_end.isoformat())
            ...
            if not time_str:
                self.pending_create = {"title": title, "duration": duration, "waiting_for": "time"}
            ...
            try:
                ...
                result, error = self.create_event(title, start_time.isoformat(), duration)
                if error:
    ...
    async def handle_pending_invite(self, user_input: str):
        ...
        if any(phrase in lower for phrase in ["never mind", "nevermind", "cancel", "forget it", "no", "nope", "don't", "dont"]):
            self.pending_invite = None
        ...
        if waiting_for == "event":
            ...
            if event:
                ...
                if stored_email:
                    ...
                    email_spoken = stored_email.replace("@", " at ").replace(".", " dot ")
                    self.pending_invite = {"event": event, "email": stored_email, "waiting_for": "confirm"}
            ...
            if any(phrase in lower for phrase in ["yes", "yeah", "yep", "sure", "okay", "ok", "correct", "right", "do it", "go ahead", "sounds good", "that's right", "thats right"]):
                self.pending_invite = None
            ...
            if corrected_email and corrected_email != email:
                ...
                email_spoken = corrected_email.replace("@", " at ").replace(".", " dot ")
                self.pending_invite = {
                    "event": event,
    ...
    async def handle_pending_create(self, user_input: str):
        ...
        if any(phrase in lower for phrase in ["never mind", "nevermind", "cancel", "forget it"]):
            self.pending_create = None
            return "Okay, I've cancelled that."
        ...
        if waiting_for == "title":
            ...
            try:
                ...
                formatted_time = start_time.strftime("%-I:%M %p").lower().replace(":00", "")
...
Return ONLY the time, no explanation:"""
    ...
    async def handle_pending_action(self, user_input: str):
        ...
        # Check for cancel
        cancel = any(phrase in lower for phrase in [
            "never mind", "nevermind", "cancel", "forget it", "no don't"
        ])
        ...
        if action.get("waiting_for") == "event":
            ...
            # Fallback
            self.pending_calendar_action = None
            return f"I've selected '{event['title']}'. What would you like to do with it?"
        ...
        if action.get("waiting_for") == "time_change":
            ...
            if target_time:
                ...
                try:
                    ...
                    self.log(f"Reschedule to target time: {target_time.strftime('%I:%M %p')} (delta: {change_mins}m)")
                    ...
                    conflicts = self.detect_conflicts(event["id"], target_time.isoformat(), new_end.isoformat())
                    ...
                    clean_title = self.clean_title_for_speech(event['title'])
                    formatted_time = target_time.strftime("%-I:%M %p").lower().replace(":00", "")
            ...
            move_earlier = any(w in lower for w in ["earlier", "up", "forward", "sooner", "before"])
            ...
            clean_title = self.clean_title_for_speech(event['title'])
            if change_mins < 0:
                return f"Done! I've moved '{clean_title}' up {abs(change_mins)} minutes."
            else:
        ...
        # =====================================================================
        cascade_yes = any(phrase in lower for phrase in [
            "push them", "move them", "yes", "yeah", "yep", "sure", "okay", "ok",
            "push everything", "move everything", "push all", "move all", 
            "cascade", "all of them", "do it", "both", "adjust them", "adjust those",
            "go ahead", "sounds good", "that works"
        ])
        
        shorten_pref = any(phrase in lower for phrase in [
            "shorten", "shorter", "cut it", "make it shorter", "just shorten",
            "end earlier", "end early"
        ])
        ...
        if action_type == "reschedule_with_conflict":
            ...
            if shorten_pref:
                try:
                    ...
                    next_start = datetime.datetime.fromisoformat(next_event["start"].replace("Z", "+00:00"))
                    
                    available_mins = int((next_start - event_start).total_seconds() / 60)
                    ...
                    self.pending_calendar_action = None
                    return "Had trouble calculating the new duration. Want to try again?"
        ...
        if action_type == "cancel_confirm" and action.get("waiting_for") == "confirm":
            ...
            confirm_yes = any(phrase in lower for phrase in [
                "yes", "yeah", "yep", "sure", "okay", "ok", "do it", 
                "go ahead", "confirm", "that's right", "correct"
            ])
            ...
            # Check for no
            deny = any(phrase in lower for phrase in [
                "no", "nope", "don't", "dont", "stop"
            ])
            ...
            for e in self.context["calendar"]:
                if e.get("in_progress"):
                    ...
                    if mins_until is not None and mins_until <= 30:
                        line = f"- [STARTING IN {mins_until}m] {e['start']}: {e['title']}"
                    else:
    ...
    async def boot_full(self):
        ...
        if self.last_session_timestamp:
            try:
                ts = datetime.datetime.fromisoformat(self.last_session_timestamp.replace('Z', '+00:00'))
    ...
    async def handle_quick_intent(self):
        ...
        if intent in ["read_today", "read_specific"]:
            ...
            self.session_history.append({"role": "user", "content": trigger or "calendar check"})
        ...
        if intent == "create":
            ...
            if not time_str:
                self.pending_create = {"title": title, "duration": duration, "waiting_for": "time"}
                await self.capability_worker.speak(f"Got it, '{title}'. What time should it be?")
                return
            ...
            try:
                ...
                if not start_time:
                    self.pending_create = {"title": title, "duration": duration, "waiting_for": "time"}
                    await self.capability_worker.speak(f"I couldn't catch the time. When should '{title}' be?")
                    return
                
                result, error = self.create_event(title, start_time.isoformat(), duration)
                if error:
                    await self.capability_worker.speak(f"Couldn't create the event: {error}")
                else:
                    formatted_time = start_time.strftime("%-I:%M %p").lower().replace(":00", "").replace(" 0", " ")
                    await self.capability_worker.speak(f"Done! I've created '{title}' at {formatted_time}.")
                    self.context["calendar"] = self.fetch_upcoming_today()
            except Exception as e:
                self.log_err(f"Create error: {e}")
                await self.capability_worker.speak("Had trouble creating that event. Could you try again?")
            return
        ...
        if intent == "modify":
            ...
            await self.capability_worker.speak("Which event do you want to change, and how?")
            return
        ...
        # =====================================================================
        if intent == "cancel":
            event_match = details.get("event_name") or details.get("person") or details.get("time")
            ...
            if event_match:
                ...
                if event:
                    ...
                    self.log(f"Set pending_calendar_action for confirm: {event['title']}")
                    await self.capability_worker.speak(f"Cancel '{clean_title}'? Say yes to confirm.")
                    return
            ...
            if calendar and len(calendar) <= 5:
                event_names = [self.clean_title_for_speech(e.get("title", "Untitled")) for e in calendar]
                await self.capability_worker.speak(f"Which event do you want to cancel? You have: {', '.join(event_names)}.")
            else:
        ...
        if intent == "invite":
            ...
            if not event and len(calendar) > 1:
                ...
                event_names = [self.clean_title_for_speech(e.get("title", "Untitled")) for e in calendar[:5]]
                await self.capability_worker.speak(f"Which meeting? You have: {', '.join(event_names)}.")
                return
            ...
            if not event:
                self.pending_invite = {"waiting_for": "event", "email": email}
                await self.capability_worker.speak("Which meeting should I add them to?")
                return
            
            if email:
                email_spoken = email.replace("@", " at ").replace(".", " dot ")
                self.pending_invite = {"event": event, "email": email, "waiting_for": "confirm"}
                clean_title = self.clean_title_for_speech(event['title'])
                await self.capability_worker.speak(f"Just to confirm, I'll add {email_spoken} to '{clean_title}'. Sound good?")
            else:
                self.pending_invite = {"event": event, "waiting_for": "email"}
                clean_title = self.clean_title_for_speech(event['title'])
                await self.capability_worker.speak(f"Who would you like me to add to '{clean_title}'? Just give me their email.")
            return
            ...
            if cal_intent == "create":
                ...
                if not title:
                    ...
                    if start_time:
                        result, error = self.create_event(title, start_time.isoformat(), duration)
                        if error:
                            await self.capability_worker.speak(f"Couldn't create that: {error}")
                        else:
                            formatted = start_time.strftime("%-I:%M %p").lower().replace(":00", "")
                            await self.capability_worker.speak(f"Done! Created '{title}' at {formatted}.")
                            self.context["calendar"] = self.fetch_upcoming_today()
                    else:
                        self.pending_create = {"title": title, "duration": duration, "waiting_for": "time"}
                        await self.capability_worker.speak(f"Couldn't catch the time. When should '{title}' be?")
                return
                ...
                if event:
                    self.pending_invite = {"event": event, "waiting_for": "email"}
                    await self.capability_worker.speak(f"Who should I add to '{event['title']}'? Just give me their email.")
                else:
                    await self.capability_worker.speak("Which meeting should I add someone to?")
                return
                ...
                if events:
                    ...
                    await self.capability_worker.speak("You don't have any more events today.")
                return
        
        # =====================================================================
        # FALLBACK - shouldn't hit this in quick mode
        # =====================================================================
        await self.capability_worker.speak("I'm not sure what you need. Could you say that again?")
    ...
    async def quick_answer_loop(self):
        ...
        has_pending = self.pending_create or self.pending_invite or self.pending_calendar_action
        self.log(f"Quick loop: has_pending={has_pending} (create={bool(self.pending_create)}, invite={bool(self.pending_invite)}, action={bool(self.pending_calendar_action)})")
        
        if not has_pending:
            # No pending state - ask if there's anything else
            await self.capability_worker.speak("Let me know if you have any other questions about your calendar.")
        ...
        if not has_pending:
            noise_words = ["um", "uh", "hmm", "okay", "ok", "alright", "sure", "thanks", "thank you", "cool", "great"]
    ...
    async def session_loop(self):
        ...
        while True:
            ...
            if self.pending_invite:
                self.log(f"Pending invite: waiting for {self.pending_invite.get('waiting_for')}")
                invite_response = await self.handle_pending_invite(user_input)
                if invite_response:
                    self.log(f"Invite response: {invite_response[:100]}")
                    self.session_history.append({"role": "user", "content": user_input})
                    self.session_history.append({"role": "assistant", "content": invite_response})
            ...
            if self.pending_create:
                self.log(f"Pending create: waiting for {self.pending_create.get('waiting_for')}")
                create_response = await self.handle_pending_create(user_input)
                if create_response:
                    self.log(f"Create response: {create_response[:100]}")
                    self.session_history.append({"role": "user", "content": user_input})
                    self.session_history.append({"role": "assistant", "content": create_response})
            ...
            if self.pending_calendar_action:
                self.log(f"Pending action exists: {self.pending_calendar_action.get('type', 'unknown')}")
                pending_response = await self.handle_pending_action(user_input)
                if pending_response:
                    self.log(f"Pending action response: {pending_response[:100]}")
                    self.session_history.append({"role": "user", "content": user_input})
                    self.session_history.append({"role": "assistant", "content": pending_response})
            ...
            if intent_data.get("intent") != "none":
                write_response = await self.handle_calendar_write(intent_data, user_input)
                if write_response:
                    self.log(f"Calendar write response: {write_response[:100]}")
                    self.session_history.append({"role": "user", "content": user_input})
                    self.session_history.append({"role": "assistant", "content": write_response})
    ...
    async def run_hub(self):
        ...
        try:
            ...
            for attempt in range(6):  # 6 attempts x 0.5s = 3 seconds max
                ...
                try:
                    ...
                    if current_len > initial_history_len:
                        self.log(f"History updated: {initial_history_len} -> {current_len}")
            ...
            if self.session_mode == "quick":
                ...
                if self.pending_create or self.pending_invite or self.pending_calendar_action:
