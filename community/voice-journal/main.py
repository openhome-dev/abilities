...
class VoiceJournalCapability(MatchingCapability):
    ...
    async def _handle_edit(self):
        ...
        try:
            ...
            # Show last 5 entries numbered
            display_lines = lines[-5:]
            offset = len(lines) - len(display_lines)
            parts = []
            for i, line in enumerate(display_lines, 1):
                formatted = self._format_entry_for_speech(line)
                parts.append(f"Number {i}: {formatted}")
            await self.capability_worker.speak(
                "Here are your recent entries. " + " ".join(parts)
            )
            ...
            # Map display number to actual line index
            actual_idx = offset + (num - 1)
    ...
    async def _handle_help(self):
        ...
        await self.capability_worker.speak(
            ...
            "something, edit an entry, or delete everything. Say done to leave."
        )
