...
class StockCapability(MatchingCapability):
    ...
    def _build_briefing_from_results(self, results: List[str]) -> str:
        """
        Build a short briefing from the results list.
        """
    ...
    async def run_main(self):
        try:
            ...
            """
            raw_t = self.capability_worker.text_to_text_response(extraction_prompt)
            raw_tickers = raw_t.strip().upper().replace('"', '')
            ...
            if any(word in lower_msg for word in ["remove", "delete", "delet"]):
                if any(x in lower_msg for x in ["remove all", "delete all", "clear"]):
                    if portfolio:
                        ...
                        await self.capability_worker.speak("All removed.")
                    else:
                ...
                if extracted_list:
                    ...
                    if removed_any:
                        await self.save_portfolio(portfolio)
                        await self.capability_worker.speak(f"Removed {', '.join(removed_items)}.")
                    else:
                        await self.capability_worker.speak("Not found in portfolio.")
                    return
                else:
                    await self.capability_worker.speak("Tell me which stocks to remove.")
                    return
