...
class StockCapability(MatchingCapability):
    ...
    async def save_portfolio(self, stocks: List[str]):
        ...
        self.worker.editor_logging_handler.info(f"Portfolio saved: {stocks}")

    # --- REAL-TIME DATA ENGINE ---
    def fetch_real_price(self, symbol: str) -> str:
        """Fetch stock price or use LLM fallback."""
        ...
        try:
            ...
            # Backup using native LLM Search
    ...
    async def run_main(self):
        try:
            ...
            raw_tickers = self.capability_worker.text_to_text_response(extraction_prompt)
            raw_tickers = raw_tickers.strip().upper().replace('"', '')
            ...
            if any(word in lower_msg for word in ["remove", "delete", "delet"]):
                ...
                if extracted_list:
                    ...
                    if removed_any:
                        ...
                        await self.capability_worker.speak("Those were not in your list.")
                    return
            ...
            await self.capability_worker.speak("I had a problem connecting to data.")
        finally:
