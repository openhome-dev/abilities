    ...
    async def watch_loop(self):
        ...
        while True:
            try:
                ...
                # Morning alert — fires once per day at morning_time
                if current_hhmm == morning_time and morning_alerted_date != today:
                    ...
                    if aq and aq["current_aqi"] >= threshold:
                        ...
                        morning_alerted_date = today
                        self.worker.editor_logging_handler.info(
                            f"[AQAlertBG] Morning check — AQI {aq['current_aqi']:.0f} below threshold, no alert"
                        )

                # Evening prep — fires once per day at evening_time if tomorrow looks bad
