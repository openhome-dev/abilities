...
class AsanaProjectManagerCapability(MatchingCapability):
    ...
    async def detect_mode(self, command: str) -> str:
        ...
        # Search Task - improved detection
        if any(word in cmd_lower for word in [
            "find the", "find a", "find ", "look up", "search for", "search the", "search a"
        ]):
            return "search_task"
    ...
    async def fetch_my_tasks(
        ...
        if filter_type == "today":
            ...
            tasks = [
                t for t in tasks
                if t.get("due_on")
                and datetime.strptime(t["due_on"], "%Y-%m-%d").date() <= week_end
            ]
            ...
            tasks = [
                t for t in tasks
                if t.get("due_on")
                and datetime.strptime(t["due_on"], "%Y-%m-%d").date() < today
            ]
    ...
    async def assign_task(
        ...
        await self.capability_worker.speak(
            "Assigning tasks to other users is not yet implemented. "
            "You can do this in the Asana app."
        )
