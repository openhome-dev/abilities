...
class StockCapability(MatchingCapability):
    ...
    async def get_portfolio(self) -> List[str]:
        ...
        if exists:
            ...
            try:
                ...
                # malformed JSON — return empty portfolio
                return []
        return []
    ...
    async def save_portfolio(self, stocks: List[str]) -> None:
        ...
        # DELETE the old file (write_file appends in some backends)
        ...
        await self.capability_worker.write_file(
            self.FILENAME, json.dumps(data_to_save), self.PERSIST
        )
    ...
    def fetch_real_price(self, symbol: str) -> str:
        ...
        try:
            ...
            self.worker.editor_logging_handler.error(
                f"fetch_real_price error for {symbol}: {e}"
            )
    ...
    def _build_briefing_from_results(self, results: List[str]) -> str:
        """
        Deterministically build a short briefing (max 2 sentences) from the raw
        results list, converting decimals to 'point' pronunciation.
        """
        ...
        remainder = phrases[mid:]
        second_sentence = (
            ". ".join(p.rstrip(".") for p in remainder) + "." if remainder else ""
        )
    ...
    async def run_main(self) -> None:
        try:
            ...
            if history:
                msg_obj = history[-1]
                user_msg = (
                    msg_obj.content if hasattr(msg_obj, "content") else str(msg_obj)
                )
            ...
            if any(word in lower_msg for word in ["remove", "delete", "delet"]):
                ...
                if (
                    ...
                    self.worker.editor_logging_handler.info(
                        "Performed 'remove all' on portfolio."
                    )
                    return
            ...
            if not tickers_to_check:
                ...
                # Build deterministic, TTS-friendly briefing
