...
## CapabilityWorker Canonical Method List

Use this as the SDK source-of-truth method inventory.

```python
__init__(capability, llm_client="", llm_model_name="")
send_interrupt_signal()
write_local_command(command, target_id=None)
read_local_response(clear=True)
exec_local_command(command, target_id=None, timeout=60.0)
gather_streamed_response(text_iterator)
get_full_message_history()
generate_ttt_using_openrouter(prompt_text, system_prompt="", history=[])
text_to_text_response(prompt_text, history=[], system_prompt="")
llm_search(query, system_prompt="", history=[])
llm_tools(query, tools, system_prompt="", history=[])
text_to_speech(prompt, voice_id)
fixed_text_iterator(text_list)
play_audio(file_content)
play_from_audio_file(file_name=None)
...
send_devkit_mqtt_action(topic="", action="", value="", command="")
send_notification_to_ios(title="", body="", time_interval=1)
send_agent_message_without_audio(value)
get_slack_key()
get_token(platform)
get_user_email_address()
resume_normal_flow()
update_personality_agent_prompt(prompt_addition)
get_live_editor_capability_path()
get_timezone()
send_email(host, port, sender_email, sender_password, receiver_email, cc_emails, subject, body, attachment_paths=[])
```
